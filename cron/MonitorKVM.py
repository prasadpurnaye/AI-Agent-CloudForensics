...
def DomainInformation(vmid,conn,mycursor):
	tuple_String=""
#	tuple_String="\n"
...
#	tuple_String +=str(err)		#vda_errors
	...
	print("the res is")
	print(res)
	sql = "INSERT INTO HypervisorMonitoringDB ( LAST_POLL, VMID, UUID, dom, rxbytes, rxpackets,  txbytes, txpackets, timecpu, timesys, timeusr, memmajor_fault, memminor_fault, memusable, memlast_update, vdard_req, vdard_bytes, vdawr_reqs, vdawr_bytes, hdard_req, hdard_bytes, hdawr_reqs, hdawr_bytes,Status ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
if len(domainIDs) == 0:
    ...
    for domainID in domainIDs:
        DomainInformation(domainID,conn,mycursor)
    time.sleep(30)
